...
int main(void)
{
  ...
  MX_RTC_Init();
  /* USER CODE BEGIN 2 */
  /* SPI1: CubeMX supplies MSP + GPIO; libtropic lt_port_init() calls HAL_SPI_Init(). */
  (void)hspi1;
  Secure_WolfSSL_Init();
...
static void MX_RTC_Init(void)
{
  ...
  */
  ...
  /* Wall clock is RAM + SysTick, set via PROVISION / ENCRYPT / DECRYPT <unix>. */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : PWR_Pin CS_Pin */
  GPIO_InitStruct.Pin = PWR_Pin|CS_Pin;
