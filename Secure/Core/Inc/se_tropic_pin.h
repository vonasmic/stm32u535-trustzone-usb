 ...
 * @brief   MAC-and-Destroy PIN setup/check (wolfCrypt HMAC-SHA384)
 *
...
/**
 * PIN attempt pairs used by this scheme (max TR01_MACANDD_ROUNDS_MAX = 128
 * hardware slots). Every wrong attempt destroys two slots permanently;
 * exhausting the budget makes the KEK — and therefore every record stored
 * under it — unrecoverable. The host model overrides this to 4 in
...
#define SE_TROPIC_PIN_MD_PER_TRY 2u
#define SE_TROPIC_PIN_MD_SLOTS   (SE_TROPIC_PIN_ROUNDS * SE_TROPIC_PIN_MD_PER_TRY)

/** HMAC-SHA384 digest / master_secret / wrap / final_key width. */
#define SE_TROPIC_PIN_HMAC_LEN 48u
...
#if (SE_TROPIC_PIN_MD_SLOTS > TR01_MACANDD_ROUNDS_MAX)
#error "SE_TROPIC_PIN_MD_SLOTS exceeds Tropic M&D slot count"
#endif
...
/**
 * Check PIN. Wrong PIN permanently consumes one M&D slot pair until a correct
 * PIN re-initializes the scheme (or the budget is exhausted).
 */
